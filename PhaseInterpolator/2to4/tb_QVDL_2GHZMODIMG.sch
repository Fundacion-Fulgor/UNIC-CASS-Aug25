...
N 2168.75 -950 2550 -950 {lab=#net1}
N 2550 -950 2550 -840 {lab=#net1}
...
* Medir Retardo (Delta T entre señales)
.meas tran T_delay 
+ TRIG v(vin1) VAL=0.6  RISE=3
+ TARG v(vout90) VAL=0.6  RISE=3

* Calcular Fase en Grados
.meas tran Phase_Deg90 param = '(T_delay/T_period)*360'

**Desfasaje 270ª**
* Medir Periodo (Referencia)
.meas tran T_per90
+ TRIG v(vout90) VAL=0.6 RISE=3
+ TARG v(vout90) VAL=0.6 RISE=4

* Medir Diferencia (Esperado: T_per/2)
.meas tran T_diff
+ TRIG v(vout90) VAL=0.6 RISE=3
+ TARG v(vout270) VAL=0.6 RISE=3

* Calcular Grados (Esperado: 180°)
.meas tran PhaseDiff
+ param = '(T_diff/T_per90)*360'

.control
run
set color0=white
*plot v(vin1) v(vout90)
plot v(vout90) v(Vout270)
...
C {devices/vsource.sym} 2382.5 -765 0 0 {name=Vdd3 value=0
spice_ignore=true}
...
C {devices/launcher.sym} 2000 -1080 0 0 {name=h5
descr="load waves Ctrl + left click" 
tclcommand="xschem raw_read $netlist_dir/tran_logic.raw tran"
}
C {devices/launcher.sym} 2018.125 -1028.75 0 0 {name=h1
descr="Simulate" 
tclcommand="xschem save; xschem netlist; xschem simulate"
}
...
C {/foss/designs/modificaciones/large_delay_vto1p1.sym} 2110 -800 0 0 {name=x2}
