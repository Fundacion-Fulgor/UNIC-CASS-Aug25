...
# python3 asm_to_bin.py test.asm output.bin
...
class Assembler:
    # Returns a list with the tokens for each instruction
    def tokenizer(self, asm_file):
        lines = asm_file.readlines()
        tokens = []
        self.labels = {}
        instruction_index = 0
        # label_count = 0  # ← Counts how many labels appeared so far
        ...
        for line in lines:
            ...
            if ':' in line:
                ...
                # self.labels[label] = instruction_index + label_count  # ← adds 1 for each previous label
                self.labels[label] = instruction_index   # ← store the instruction index for the label
                # label_count += 1
                line = rest.strip()
                if not line:
                    continue
    ...
    # Takes one of the numbers of the instruction or the register number 
    # and converts it to a binary string.
    # Example: SLL r1, 2, -3 → if we ask for sa = "11101"
    def str_to_bin_str(self, str, n_bits):
        bin_str = ''

        matches = re.search(r'R{0,1}(-{0,1}\d+)', str)
        if matches == None:
            log.fatal(f'Could not match any value for str = {str}')
    ...
    def instruction_generator(self, token):
        ...
        if i_name == "SLL":
            ...
            if label.isdigit() or (label.startswith('-') and label[1:].isdigit()):
                ...
                log.fatal(f"Label '{label}' not found for BEQ")
        elif i_name == "BNE":
            ...
            if label.isdigit() or (label.startswith('-') and label[1:].isdigit()):
                ...
                log.fatal(f"Label '{label}' not found for BNE")
        elif i_name == "J":
            ...
            if not target.isdigit():
                if target in self.labels:
                    ...
                    log.fatal(f"Label '{target}' not found")
            ...
            inst_bin = self.set_op_code(inst_bin, "000000")   # R-type
            inst_bin = self.set_func(inst_bin, "001001")     # funct = 0x09

            if len(token) > 2:
                # JALR rd, rs
                inst_bin = self.set_rd(inst_bin, token[1])   # save PC+8 in rd
                inst_bin = self.set_rs(inst_bin, token[2])   # jump to address in rs
            else:
                # JALR rs → equivalent to JALR R31, rs
            ...
            print(i_name)
            log.FATAL(f'Unrecognized instruction {i_name}')
...
asm.instruction_counter = 0  # Add this line before the loop
